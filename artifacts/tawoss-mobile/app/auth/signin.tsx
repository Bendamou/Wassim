...
export default function SignIn() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      Alert.alert(ar.error, err.message ?? ar.phoneNotFound);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <KeyboardAvoidingView style={[s.container, { paddingTop: Platform.OS === "web" ? 67 : insets.top }]} behavior={Platform.OS === "ios" ? "padding" : "height"}>
      <View style={s.content}>
        ...
        <Text style={s.title}>{ar.welcomeBack}</Text>
        <Text style={s.sub}>{ar.enterPhone}</Text>
        ...
        >
          {loading ? <ActivityIndicator color="#000" /> : (
            <>
              ...
              <Text style={s.btnText}>{ar.signIn}</Text>
            </>
          )}
        </Pressable>

        <TouchableOpacity onPress={() => router.replace("/auth")} style={s.createRow}>
          <Text style={s.createLink}>{ar.createAccountLink}</Text>
          <Text style={s.createText}> {ar.newHere}</Text>
        </TouchableOpacity>
      </View>
...
const s = StyleSheet.create({
  ...
  title: { fontSize: 26, fontFamily: "Cairo_700Bold", color: "#f0eeff", marginBottom: 6, marginTop: 8, textAlign: "right" },
  sub: { fontSize: 14, fontFamily: "Cairo_400Regular", color: "#9ca3af", marginBottom: 28, textAlign: "right" },
  inputRow: {
