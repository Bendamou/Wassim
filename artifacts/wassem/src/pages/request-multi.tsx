...
const CAT_COLORS: Record<string, string> = {
  barber: "#00B4FF", hair: "#00B4FF",
  nails: "#FF1F8E", skincare: "#FF1F8E", massage: "#FF1F8E", spa: "#FF1F8E",
};
...
export default function RequestMulti() {
  ...
  return (
    <div className="min-h-[100dvh] bg-[#090013] pb-32">
      {/* Header */}
      <div className="bg-gradient-to-b from-[#FF1F8E]/10 to-transparent pt-12 pb-5 px-5">
        <div className="flex items-center gap-3 mb-1">
          ...
          <div>
            <p className="text-[#FF1F8E] text-xs font-bold uppercase tracking-widest">Multi-Book</p>
          ...
          {["salon", "services", "confirm"].map((s, i) => (
            <div key={s} className="flex items-center gap-2">
              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-black ${
                step === s ? "bg-[#FF1F8E] text-black" :
                ["salon","services","confirm"].indexOf(step) > i ? "bg-[#FF1F8E]/30 text-[#FF1F8E]" : "bg-white/10 text-gray-600"
              }`}>
                {["salon","services","confirm"].indexOf(step) > i ? <Check size={12} /> : i + 1}
              </div>
              {i < 2 && <div className={`h-0.5 w-8 ${["salon","services","confirm"].indexOf(step) > i ? "bg-[#FF1F8E]/40" : "bg-white/10"}`} />}
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="px-4 space-y-3">
          {salons.map(salon => (
            <button
              ...
              className="w-full bg-white/5 border border-white/10 hover:border-[#FF1F8E]/50 rounded-2xl p-4 text-left transition-all"
            >
              <div className="flex items-center justify-between">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {salon.is_verified && (
                      <div className="flex items-center gap-0.5 bg-[#00B4FF]/20 border border-[#00B4FF]/40 rounded-full px-1.5 py-0.5">
                        <Check size={9} className="text-[#00B4FF]" />
                        <span className="text-[#00B4FF] text-[9px] font-bold">Verified</span>
                      </div>
                    )}
                  </div>
        ...
        <div className="px-4">
          ...
          {selected.size > 0 && (
            <button
              ...
              className="w-full bg-[#FF1F8E] text-black font-black text-lg rounded-2xl py-4 shadow-[0_0_25px_rgba(255,0,255,0.4)] flex items-center justify-center gap-2 mt-2"
            >
      ...
      {step === "confirm" && selectedSalon && (
        <div className="px-4 space-y-4">
          <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
            ...
            <div className="border-t border-white/10 mt-4 pt-4">
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-[#FF1F8E] font-black text-xl">{totalPrice} MAD</span>
              </div>
              <div className="flex items-center justify-between">
          ...
          <div className="bg-[#FF1F8E]/10 border border-[#FF1F8E]/30 rounded-2xl p-4 flex items-center gap-3">
            <Zap size={18} className="text-[#FF1F8E] flex-shrink-0" />
            <p className="text-gray-300 text-sm">
              This is an <span className="text-[#FF1F8E] font-bold">Urgent Request</span> — ghost professionals near <span className="font-bold text-white">{selectedSalon.name}</span> will start bidding immediately.
            </p>
          </div>
          ...
          <button
            ...
            disabled={submitting}
            className="w-full bg-[#FF1F8E] disabled:opacity-50 text-black font-black text-xl rounded-2xl py-5 shadow-[0_0_30px_rgba(255,0,255,0.5)] flex items-center justify-center gap-2"
          >
