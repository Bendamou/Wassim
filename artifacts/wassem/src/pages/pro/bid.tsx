  ...
  return (
    <div className="min-h-[100dvh] bg-[#0A0A0A] flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-36 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            ...
            className={`rounded-2xl p-5 border-2 transition-all text-left ${
              mode === "accept"
                ? "bg-[#00C1FF]/10 border-[#00C1FF] shadow-[0_0_20px_rgba(0,193,255,0.2)]"
                : "bg-white/5 border-white/10"
            }`}
          >
            <Check size={24} className={mode === "accept" ? "text-[#00C1FF]" : "text-gray-500"} />
          ...
          >
            <MessageSquare size={24} className={mode === "counter" ? "text-[#FF00FF]" : "text-gray-500"} />
        ...
        {mode === "counter" && (
          <div className="bg-white/5 border border-[#FF00FF]/20 rounded-3xl p-6">
            ...
            <div className="flex items-center justify-center gap-6">
              ...
              <button
                ...
                className="w-14 h-14 rounded-full bg-[#FF00FF] flex items-center justify-center text-white hover:bg-[#e600e6] active:scale-95 shadow-[0_0_20px_rgba(255,0,255,0.4)]"
              >
                <Plus size={24} />
              </button>
            </div>
            {counterPrice !== job.budget && (
              <p className={`text-center text-sm font-bold mt-3 ${counterPrice < job.budget ? "text-green-400" : "text-[#FF00FF]"}`}>
        ...
        <div>
          ...
          <div className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-2xl px-4 py-4 focus-within:border-[#FF00FF] transition-colors">
            <Clock size={18} className="text-[#FF00FF] flex-shrink-0" />
            <input
              value={eta}
      ...
      <div className="fixed bottom-0 left-0 right-0 p-5 bg-gradient-to-t from-[#0A0A0A] via-[#0A0A0A] to-transparent pb-safe-bottom">
        <button
          ...
          className={`w-full font-black text-xl rounded-2xl py-5 transition-all flex items-center justify-center gap-2 active:scale-[0.97] disabled:opacity-40 ${
            mode === "accept"
              ? "bg-[#00C1FF] hover:bg-[#00b0e8] text-black shadow-[0_0_30px_rgba(0,193,255,0.5)]"
              : "bg-[#FF00FF] hover:bg-[#e600e6] text-white shadow-[0_0_30px_rgba(255,0,255,0.5)]"
          }`}
        >
